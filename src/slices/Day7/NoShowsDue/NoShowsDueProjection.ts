...
export const tableName = 'day7_no_shows_due';

export const GRACE_MINUTES = 15;

export type NoShowsDueReadModel = {
...
export const sortableStampOf = (date: string, time: string): string | null => {
    const at = wallClock(date, time);
    return at === null ? null : sortableStamp(at);
};

export const graceEndsAt = (date: string, startTime: string): number | null => {
    const start = wallClock(date, startTime);
    return start === null ? null : start + GRACE_MINUTES * 60 * 1000;
};

type NoShowsDueEvents = ReservationConfirmed | ReservationReleasedAsNoShow | ReservationCancelled;

export const NoShowsDueProjection = postgreSQLRawSQLProjection<NoShowsDueEvents>({
    name: 'NoShowsDueProjection',
    canHandle: ['ReservationConfirmed', 'ReservationReleasedAsNoShow', 'ReservationCancelled'],
    evolve: async (event): Promise<SQL[]> => {
        const db = getKnexInstance();

        switch (event.type) {
            case 'ReservationConfirmed': {
                const expiry = graceEndsAt(event.data.date, event.data.startTime);
                ...
                return [sql(db(tableName)
                    ...
                    .insert({
                        ...
                        grace_ends_at: formatStamp(expiry),
                        grace_ends_at_sortable: sortableStamp(expiry),
                    })
                    .onConflict('reservation_code')
                    .merge(['e_mail', 'date', 'start_time', 'table_number', 'grace_ends_at', 'grace_ends_at_sortable'])
                    .toQuery())];
            }

            // The emitted event closes back onto the list it was drained from, so a
            // released reservation is not offered to the automation a second time.
            case 'ReservationReleasedAsNoShow':
            // A cancelled reservation is not a no-show — the guest told us they were
            // not coming, so it leaves the list rather than waiting out the grace period.
