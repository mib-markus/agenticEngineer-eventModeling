...
import {ChangeOrderLineCommand, handleChangeOrderLine} from './ChangeOrderLineCommand';
...
export const api = (): WebApiSetup => (router: Router): void => {
    ...
    /**
     * @openapi
     * /api/day12/changeorderline/{orderNumber}:
     *   post:
     *     tags: [Day12]
     *     summary: Change Order Line
     *     description: >
     *       An existing line on an open pad has its quantity and/or special wishes
     *       revised. A line that is not on the pad, a line already struck off, or any
     *       line on an order that was already submitted to the kitchen cannot be changed.
     *     parameters:
     *       - in: path
     *         name: orderNumber
     *         required: true
     ...
     *           type: string
     *           example: O-1042
     *         description: The open order whose line is being changed
     *       - in: header
     *         name: correlation_id
     *         required: false
     *         schema:
     ...
     *           schema:
     *             type: object
     *             required: [lineNumber, quantity, specialWishes]
     *             properties:
     *               lineNumber:
     *                 type: integer
     *                 format: int32
     ...
     *     responses:
     *       '201':
     *         description: Accepted — OrderLineChanged appended
     *         content:
     *           application/json:
     *             schema:
     *               type: object
     ...
     */
    router.post('/api/day12/changeorderline/:orderNumber', async (req: Request, res: Response) => {
        ...
        try {
            ...
            const command: ChangeOrderLineCommand = {
            ...
            const result = await handleChangeOrderLine(tableNumber, command);
